...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return redirectWithFeedback(req, "/admin/pengurus", { ok: "Data pengurus berhasil dihapus." });
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
      return redirectWithFeedback(req, "/admin/pengurus", { err: "Data pengurus tidak ditemukan." });
    }
    return redirectWithFeedback(req, "/admin/pengurus", { err: "Gagal menghapus data pengurus." });
  }
}
