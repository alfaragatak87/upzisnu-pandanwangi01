...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return redirectWithFeedback(req, "/admin/galeri", { ok: "Foto galeri berhasil dihapus." });
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
      return redirectWithFeedback(req, "/admin/galeri", { err: "Foto galeri tidak ditemukan." });
    }
    return redirectWithFeedback(req, "/admin/galeri", { err: "Gagal menghapus foto galeri." });
  }
}
